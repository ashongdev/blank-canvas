  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <p className="text-sm text-muted-foreground">{templates.length} item{templates.length !== 1 ? "s" : ""}</p>
      </div>
      ...
      {templates.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          ...
          <p className="text-sm">Deleted templates will appear here.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {templates.map((t) => (
            <Card key={t.id} className="border-border opacity-75 hover:opacity-100 transition-opacity">
